...
dt_list = ["250525_full_features_target_classic_MP",
           "250525_full_features_target_p_exclude_ligand_multiplicities_binned3_MP"
           ]
dt = sys.argv[1]
idx = sys.argv[2]
assert dt in dt_list

mp_feat = pd.read_json(os.path.join("data", f"{dt}train{idx}.json"))
target_name = dt.removesuffix("_MP").removeprefix("250525_full_features_")

# Sanity check -> no duplicate and constant feats
df_copy = mp_feat.copy()
df_copy = df_copy.T.drop_duplicates().T
df_copy = df_copy.loc[:, (df_copy != df_copy.iloc[0]).any()]
assert df_copy.shape == mp_feat.shape

feat = mp_feat.drop(columns=[target_name])
...
for n_sampling_idx, n_sampling in enumerate(n_sampling_per_size_list):
    ...
    for rs in rs_list[lb:n_sampling]:
        ...
        for fe in mutual_info.index:
            ...
            mutual_info.at[fe, f"entropy_rs{rs}"] = mutual_info_regression(feat[fe].values.reshape(-1, 1),
                                                                            feat[fe],
                                                                            random_state=rs)[0]
        target_entropy = mutual_info_classif(mp_feat[target_name].values.reshape(-1, 1),
                                             mp_feat[target_name],
                                             random_state=rs)[0]
    ...
    if co[False] < len(mutual_info) / 100:
        ...
        mutual_info.to_json(os.path.join("data", f"250525_NMI_MI_H_{dt}train{idx}.json"))
        with open(os.path.join("data", f"250525_target_entropies_{dt}train{idx}.json"), "w") as f:

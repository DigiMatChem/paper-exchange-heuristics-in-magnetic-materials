...
get bond angle occurrences between magnetic sites as a function of the spin angle (FM / AFM).
"""
import json
    ...
    for row_id, row in df.iterrows():
        ...
        try:
            ...
            zero_magmom_failures[zero_magmom_threshold].append(
                (str(e), row_id, row["task_id"], structure.as_dict(), row["magmoms"]["vasp"]))
            continue
    ...
    for datastring in ["connected_TM_sites",
        ...
        for ligand_multiplicity_bool, ligand_multiplicity_string in zip([True, False],
            ...
            for normalize_bool, normalize_string in zip([False, True], ["absolute_occurrences", "normalized_occurrences"]):
                ...
                with open(f"data_and_plots/MP_spin_bond_angle_trends_{zero_magmom_threshold}"
...
with open("data_and_plots/zero-magmom-failures.json", "w") as f:
    json.dump(zero_magmom_failures, f)
with open("data_and_plots/ks_test_results.json", "w") as f:
    json.dump(ks_dict, f)
with open("data_and_plots/bond_angle_occurrences.json", "w") as f:

...
with open(os.path.join(mp_data_dir, "unique_4559_cnfeat_MP_db_from_api.json")) as f:
...
for magndata_id, magndata_row in df_md.iterrows():
    ...
    for mp_id, mp_row in df_mp.iterrows():
        struct_mp = mp_row["structure_dict"].copy()
        struct_mp.remove_site_property(property_name="magmom")
        ...
        if StructureMatcher().fit(struct_md, struct_mp):
            ...
            if magndata_p >= 0:
                ...
                mp_mag_structure = mp_row["structure_dict"].copy()
...
match_df.to_json(os.path.join(overlap_data_dir, "matches_mp_via_api_magndata.json"))

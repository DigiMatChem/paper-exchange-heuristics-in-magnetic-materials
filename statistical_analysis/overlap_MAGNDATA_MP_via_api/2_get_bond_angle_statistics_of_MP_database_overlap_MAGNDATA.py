...
match_df = pd.read_json(os.path.join(overlap_data_dir, "matches_mp_via_api_magndata.json"))
...
# MP
with open(os.path.join("../MP_via_api/data", "unique_4559_cnfeat_MP_db_from_api.json")) as f:
    df_mp = json.load(f, cls=MontyDecoder)

# MP imports
with open(os.path.join("../MP_via_api/data", "dfs_of_magnetic_edge_information_0p5_zero-magmom_threshold.json")) as f:
...
for normalize_bool, normalize_string in zip([False, True], ["absolute_occurrences", "normalized_occurrences"]):
    ...
    for d, d_name in zip([dist_mp, dist_md], ["MP", "MAGNDATA"]):
        ...
        if normalize_bool:
            fig.update_layout(yaxis_range=[0, 8.7], xaxis_range=[62, 182])
        else:
            fig.update_layout(yaxis_range=[0, 105], xaxis_range=[62, 182])
